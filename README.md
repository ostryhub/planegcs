...
`multiplicities` arrays. `knots` stores unique knot values, while
`multiplicities` stores the repetition count for each knot.  `degree`
specifies the order of the curve and setting `periodic` to `true` creates
a closed spline.

```ts
{ id: '12', type: 'bspline',
  start_id: START_POINT_ID, end_id: END_POINT_ID,
  control_points: [START_POINT_ID, CP1_ID, CP2_ID, END_POINT_ID],
  weights: [1, 1, 1, 1],
  knots: [0, 1],
  multiplicities: [4, 4],
...
### Curves And Bezier Lowering

The wasm solver exposes B-spline geometry. Bezier curves are represented
as a convenience sketch primitive and are lowered to an equivalent
non-periodic B-spline before they reach PlaneGCS:

```ts
const bezierControlPoints = [P0_ID, P1_ID, P2_ID, P3_ID];
...
This matches the FreeCAD Sketcher solver contract audited in
`docs/freecad-audit-notes.md`: Bezier is not a separate PlaneGCS solver
primitive; it is solved through the B-spline/NURBS representation.
